...
"""The keys for canvas will be (x, y) integer tuples for the coordinates
...
while True: # Main program loop.
    ...
    if response == 'QUIT':
        ...
        print()
        print('You can save your drawing as a text file by entering F.')
    ...
    for command in response:
        ...
        if command == 'W' and cursorY > 0:
            ...
            canvas[(cursorX, cursorY)].add(command)
            cursorX = cursorX - 1
        elif command == 'D' and cursorY < CANVAS_WIDTH - 1:
            canvas[(cursorX, cursorY)].add(command)
            cursorX = cursorX + 1
        else:
