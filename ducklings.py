...
# newline automatically, so reduce th width by one:
WIDTH -= 1
...
class Duckling:
    ...
    def getHeadStr(self):
        ...
        if self.body == CHUBBY:
            # Get and extra space so chubby ducklings are the same
    ...
    def getFeetStr(self):
        """Returns teh string of the duckling's feet."""
        if self.body == CHUBBY:
            return ' ^^ '
        elif self.body == VERY_CHUBBY:
            return ' ^ ^ '
    ...
    def getNextBodyPart(self):
        ...
        part that needs to be displayed. Sets partToDisplay to 
        None when finished."""
